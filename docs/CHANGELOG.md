...
- Fixed bilingual mode sometimes returning a **translation** instead of a transcription (e.g. Chinese dictation coming out as English). The transcription step was never at fault (it always calls the transcribe endpoint); the AI-enhancement step translated because `resolve_language` snapped a missing/out-of-pair detection to the pair's **first slot**, and the default model (`gpt-4o-mini-transcribe`) never returns a detection (`verbose_json` is suppressed for gpt-4o models) — so with "English + Chinese" configured, enhancement received the registry's forced-language instruction ("respond entirely in this language, regardless of what language the user spoke") for `en` and dutifully translated the Chinese transcript. Intermittency: the 3× length guard swallowed longer translations, and Light/Auto modes were immune. Now the **transcript's own script** decides first (`script_language`: Unicode-script classification, CJK chars weighted 3×, ≥50% weighted dominance, kana discriminates ja/zh pairs), overriding even a wrong in-pair provider detection; an in-pair detection is kept only when script evidence is inconclusive (same-script pairs like en+de); everything else resolves to `None`, which the frontend maps to the language-preserving auto instruction instead of the old snap — `enhance()` no longer falls back to the pair's first slot. T→S conversion falls back to its existing kana heuristic when unresolved.
- Fixed silence/near-silence being transcribed as hallucinated text ("Thank you for watching", subtitle-credit spam). Two layers: (1) a conservative speech gate in `AudioRecorder::stop` (`is_speechless`: < 250 ms, peak < 0.02, or RMS < 0.0015 → new `AudioError::NoSpeech`, no network call; the frontend resets silently with no error toast — typical quiet speech RMS ≥ 0.01 sits well above the floors); (2) a whole-output blocklist (`transcription/hallucination.rs`) of ~20 canonical Whisper hallucination phrases (EN/ZH/JA/KO, incl. repeated-phrase loops), matched on a normalized alphanumeric form after echo stripping and blanked so the existing empty-transcription skip applies. Phrases inside real speech are never touched; bare "thank you"/"you" are deliberately excluded as plausible real dictations.
- Fixed auto-paste occasionally pasting the same sentence twice. Two overlapping `stop()` calls (push-to-talk key bounce delivers two `Released` events — the `global-hotkey` crate spawns one release-poll thread per `WM_HOTKEY`) both passed the stale closure-state `phase` check, and `AudioRecorder::stop`'s load-then-store on `is_recording` let both callers clone the identical sample buffer → same WAV transcribed and pasted twice. Now a synchronous `stopInFlightRef` guards re-entry in `useAudioRecording.stop()` (reset in `finally`), the Rust stop claims via `compare_exchange` so exactly one caller wins, and the sample buffer is drained (`mem::take`) rather than cloned so the same audio can never be returned twice.
...
- Whisperi is now cloud-only, with all dormant on-device model and sidecar support removed
- Dictation now fails safely when Windows rejects simulated typing, and stalled cloud requests time out instead of disabling the overlay forever
...
- Removed all on-device model support. This deletes the whisper.cpp sidecar and downloader, local transcription commands and settings, local model registries, executable permissions, build/release download steps, and associated dependencies. Cloud transcription and cloud reasoning providers are unchanged.
...
- Fixed Live mode counting text as typed even when Windows blocked or only partially accepted `SendInput`. Native typing and polish swaps now verify the exact event count and return an error on mismatch; failed chunks are not added to the destructive swap ledger, and a failed polish swap falls back to the clipboard.
- Added 15-second connect and 120-second total HTTP request limits so stalled transcription or reasoning providers cannot leave Standard mode permanently disabled in the processing state.
- Isolated optional Live notifications from WebSocket startup. Notification permission or delivery failures are now logged without stopping audio or orphaning the already-open Live session.
...
- Quitting the app mid-dictation now flushes your final words and closes the connection cleanly instead of cutting off abruptly
- Hardened Live-mode privacy: it won't type into a Windows credential or lock-screen prompt if focus moves there mid-dictation, and no part of your API keys is ever written to the app log
...
- Added graceful shutdown for quit-while-Live. Previously, quitting (tray "Quit" / `quit_app`) called `app.exit(0)` with no session signalling: the audio-pump task + provider WebSocket were abandoned, the final utterance's soft-flush was skipped, and the socket was torn down abruptly. The app now runs via `Builder::build(..)?.run(|app, event| ..)` and, on `RunEvent::ExitRequested`, `block_on`s a new `LiveSessionState::shutdown()` that signals cancel to every active session and awaits its soft-flush within a bounded per-session timeout (1.5 s), aborting any that overrun. It's a no-op (instant) when no Live session is active, so normal quit is unaffected.
- Added a secure-window guard to Live mode's per-utterance typing. `type_text_chunk` → `send_text_keystrokes` typed into whatever window had focus with no check; if focus drifted onto a credential prompt or the lock screen mid-dictation, the transcript (attacker-influenceable audio) would be injected there. It now returns 0 (suppressing the keystrokes) when the foreground window class is a known secure class (`Credential Dialog Xaml Host`, `LockScreenControlsWindow`); returning 0 keeps the frontend's typed-char counter in lockstep. Best-effort — catches dedicated secure top-level windows, not in-app password fields (child controls with no distinguishing top-level class). The post-stop swap was already protected by its strict `expected_hwnd` match.
- Stopped logging any part of an API key. `transcribe_cloud` and `process_reasoning` logged a `first4…last4` key preview at info level, which could leak a credential substring into shipped logs/bug reports (and could panic on a non-ASCII key). They now log `has_key=<bool>` only. Plaintext at-rest storage / OS-keyring migration remains the separately tracked deferred item.
...
- Added three `LiveSessionState::shutdown()` unit tests in `transcription/streaming/mod.rs`: it cancels and drains an active session, aborts an unresponsive task within the bounded timeout (so quit can't hang), and is an immediate no-op when idle.
- Added `secure_window_classes_are_detected` in `clipboard/mod.rs`: the secure-window class matcher recognises credential/lock-screen classes case-insensitively and does not flag ordinary app/terminal windows.
